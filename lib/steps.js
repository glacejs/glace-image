...
var ImageSteps = {
    ...
    checkOrMakeScreenshot: async function (imageName, opts) {
        ...
        if (CONF.compareImages) {
            testName = CONF.curTestCase ? kebab(CONF.curTestCase.name) : "";
    ...
    _canvasScreenshot: async function (imagePath, opts) {
        ...
        await this.webdriver.execute(function (cssSelector) {
    ...
    _cutElement: async function (imagePath, elementName, opts) {
        ...
        var element = await this.getElement(elementName);
 ...
 */
var getScreensDir = () => {
    var testName = CONF.curTestCase ? kebab(CONF.curTestCase.name) : "";
    return path.resolve(CONF.reportsDir, testName, "screenshots");
};
/**
 * Transform string to kebab style.
 *
 * @ignore
 * @function
 */
var kebab = string => string.trim().replace(/ +/g, '-').toLowerCase();
